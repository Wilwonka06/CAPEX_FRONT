...
function OrdersTable({ orders, onView, onEdit, loading = false }) {
  if (loading) {
    return (
      <TableSkeleton columns={5} rows={5} hasAvatar={false} hasActions={true} />
    );
  }
      ...
      <OrderDetailModal
        ...
        customer={
          detailOrder
            ? {
                nombre:
                  detailOrder.usuario?.nombre ||
                  detailOrder.clienteNombre ||
                  "Cliente",
                firstName:
                  detailOrder.usuario?.nombre ||
                  detailOrder.clienteNombre ||
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-yellow-700">
                Algunos pedidos no tienen datos de cliente asociados. Por favor,
      ...
      <EditOrderModal
        ...
        customer={
          editOrder
            ? {
                nombre:
                  editOrder.usuario?.nombre ||
                  editOrder.clienteNombre ||
                  "Cliente",
                firstName:
                  editOrder.usuario?.nombre ||
                  editOrder.clienteNombre ||
                ...
                documentNumber:
                  editOrder.usuario?.documento ||
                  `DOC-${editOrder.clienteId || "N/A"}`,
                email: editOrder.usuario?.correo || "N/A",
                phone: editOrder.usuario?.telefono || "N/A",
                address:
                  editOrder.direccion_entrega ||
                  editOrder.usuario?.direccion ||
                  "No especificada",
              }
            : null
        }
