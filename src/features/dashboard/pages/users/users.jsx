...
const sortUsers = (usersArray) => {
  ...
  sortedUsers.sort((a, b) => {
    ...
    const aIsSuperAdmin =
      a.correo && a.correo.toLowerCase() === superAdminEmail.toLowerCase();
    const bIsSuperAdmin =
      b.correo && b.correo.toLowerCase() === superAdminEmail.toLowerCase();
...
const Users = () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (targetUser) {
          ...
          console.warn(
            "⚠️ [Users] Usuario con correo heieihei183@gmail.com NO encontrado en la respuesta del backend"
          );
        }
  ...
  useEffect(() => {
    ...
    console.log(
      "🔍 [Users] Usuarios sincronizados y ordenados:",
      sortedUsers.length
    );

    // Verificar si el usuario objetivo está presente
    const targetInSorted = sortedUsers.find(
      (u) =>
        u.correo && u.correo.toLowerCase().includes("heieihei183@gmail.com")
    );
    if (targetInSorted) {
      console.log(
        "✅ [Users] Usuario objetivo presente en lista ordenada:",
        targetInSorted
      );
      console.log(
        "✅ [Users] Posición en lista:",
        sortedUsers.indexOf(targetInSorted)
      );
    } else if (
      users.some(
        (u) =>
      ...
      console.error(
        "❌ [Users] Usuario objetivo existe pero NO está en lista ordenada. Posible error en sortUsers"
      );
    }
      ...
      await executeWithToast({
        promiseFn: async () => {
          const response = await usersService.update(
            updatedUser.id_usuario || updatedUser.id,
            updatedUser
          );
          if (response.success) {
            await loadUsers();
  ...
  const handleDeleteUser = async (userId) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const hasClientAssociations =
          userComplete.hasClientAssociations || false;
        const clientAssociationsInfo =
          userComplete.clientAssociationsInfo || null;

        // Si el usuario tiene ventas u órdenes asociadas, no permitir eliminación
        if (hasClientAssociations) {
          const message =
            clientAssociationsInfo?.message ||
      ...
      showError(
        "No se pudo verificar si el usuario tiene ventas asociadas. Proceda con precaución.",
        { id: "user-verify-warning" }
      );
    }
  ...
  const handleStatusChange = async (
    ...
    if (user) {
      try {
        await executeWithToast({
          promiseFn: async () => {
            const response = await usersService.changeStatus(
              userId,
              newStatus,
              conceptoEstado
            );
        ...
        if (userData.hasClientAssociations) {
          try {
            ...
            if (userActiveOrders.length === 0) {
              console.log(
                "⚠️ [Users] Usuario marcado con asociaciones pero no tiene órdenes activas (solo pagadas). Corrigiendo..."
              );
              userData = {
                ...userData,
              ...
              console.log(
                ...
                "órdenes activas confirmadas"
              );
            }
          } catch (orderError) {
            console.warn(
              "⚠️ [Users] No se pudieron verificar las órdenes del usuario:",
              orderError
            );
  ...
  return (
    <div className="min-h-screen font-inter">
      ...
      {showDetail && selectedUser && (
        <UserDetail onClose={closes} user={selectedUser} />
      )}
