      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl relative animate-fade-in max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="sticky top-0 z-10 bg-gradient-to-r from-[#FACC15] to-[#F59E0B] text-white rounded-t-2xl flex items-center justify-between px-6 py-3 shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold m-0">Detalles del Producto</h2>
          </div>
          <button
              ...
              <img
                src={
                  product.fotos && product.fotos.length > 0 && product.fotos[0]
                    ...
                    : product.foto ||
                      product.url_foto ||
                      getDefaultProductImage(product.nombre)
                }
                alt={product.nombre}
            ...
            {product.fotos && product.fotos.length > 1 && (
              <div className="flex gap-2 mb-4">
                {product.fotos.slice(1).map((foto, index) => (
                  <div
                    key={index}
                    className="w-16 h-16 bg-white rounded-xl flex items-center justify-center shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-200 cursor-pointer"
                  >
                    <img
                  ...
                  <span className="font-semibold text-gray-800 text-xs">
                    {(() => {
                      ...
                      const precio = parseFloat(
                        product.precio_venta || product.precio || 0
                      );
                      const margen = precio - costo;
                      const pct =
                        costo > 0
                          ? ((margen / costo) * 100).toFixed(2)
                          : "0.00";
              ...
              <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                ...
                <div className="space-y-1">
                  {specs.map((spec, index) => {
                    ...
                    const valor =
                      spec.FichaTecnica?.valor || spec.valor || "Sin valor";
