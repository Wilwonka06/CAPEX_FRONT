...
const APPOINTMENT_STATES = [
  ...
  {
    nombre: "Confirmada",
    descripcion: "El establecimiento ha confirmado la disponibilidad.",
  },
];
...
/**
 * Normaliza el número de teléfono al formato requerido por el backend (+XXXXXXXXXXX)
 * @param {string} phoneNumber - Número de teléfono en cualquier formato
 * @returns {string} - Teléfono normalizado con prefijo +
 */
...
const AppointmentCreateModal = ({ fecha, onClose, onSave }) => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        let allSchedules = [];
        try {
          allSchedules = await recurringSchedulingService.getAll();
        } catch (error) {
        ...
        const normalizedProfessionals = employeesData
          .filter((emp) => {
            ...
            // Si no hay fecha seleccionada, mostrar todos los activos
            if (!formData.fecha) return true;

            // Si no hay programaciones, mostrar todos los activos
            if (allSchedules.length === 0) return true;

            // Obtener programaciones del empleado
            const programacionesEmpleado = allSchedules.filter(
              (s) =>
                (s.id_usuario === empId || s.idUsuario === empId) &&
                (s.estado === "Activa" || s.estado === "Activo")
            );

            if (programacionesEmpleado.length === 0) return false;

            // Validar que al menos una programación cubra la fecha de la cita
            const fechaCita = new Date(formData.fecha + "T00:00:00");
            const diaSemana = fechaCita.getDay();

            return programacionesEmpleado.some((prog) => {
              const fechaInicio = new Date(prog.fecha_inicio + "T00:00:00");
              const fechaFin = prog.fecha_fin
                ? new Date(prog.fecha_fin + "T00:00:00")
                : new Date("2099-12-31T00:00:00");

              // Verificar que la fecha de la cita esté dentro del rango
              if (fechaCita < fechaInicio || fechaCita > fechaFin) return false;

              // Verificar que el día de la semana coincida
              return (
                Array.isArray(prog.dias_semana) &&
                prog.dias_semana.includes(diaSemana)
              );
            });
          })
          .map((emp) => ({
  ...
  }, [formData.fecha]); // ✅ IMPORTANTE: Agregar formData.fecha como dependencia
  ...
  function validarTelefono(telefono) {
    ...
    if (!digitsOnly.test(String(telefono)))
    ...
    for (let i = 0; i < servicios.length; i++) {
      for (let j = i + 1; j < servicios.length; j++) {
        if (
          ...
          const inicioA =
            parseInt(servicios[i].inicio.split(":")[0]) * 60 +
            parseInt(servicios[i].inicio.split(":")[1]);
          const finA =
            parseInt(servicios[i].fin.split(":")[0]) * 60 +
            parseInt(servicios[i].fin.split(":")[1]);
          const inicioB =
            parseInt(servicios[j].inicio.split(":")[0]) * 60 +
            parseInt(servicios[j].inicio.split(":")[1]);
          const finB =
            parseInt(servicios[j].fin.split(":")[0]) * 60 +
            parseInt(servicios[j].fin.split(":")[1]);
          // Si se solapan
  ...
  useEffect(() => {
    ...
    if (touchedFields.servicios || formData.servicios.length > 0) {
      if (formData.servicios.length === 0) {
        setErrors((prev) => ({
          ...prev,
          servicios: "Debe agregar al menos un servicio",
        }));
      } else if (haySolapamientoServicios(formData.servicios)) {
        setErrors((prev) => ({
          ...prev,
  ...
  const calcularResumen = () => {
    if (formData.servicios.length === 0)
      return { duracion: 0, inicio: "", fin: "", total: 0 };
    ...
    const duracion = formData.servicios.reduce(
      (acc, s) => acc + Number(s.duracion || 0) * (Number(s.cantidad) || 1),
      0
    );
    const total = formData.servicios.reduce(
      (acc, s) =>
        acc + parseFormattedNumber(s.precio || 0) * (Number(s.cantidad) || 1),
      0
    );
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    let newErrors = {};
    newErrors.cliente = !formData.cliente.trim()
      ? "El nombre del cliente es requerido"
      : "";
    ...
    if (formData.servicios.length === 0) {
      ...
      const serviciosSinProfesional = formData.servicios.filter(
        (s) => !s.profesional || !s.id_empleado
      );
      if (serviciosSinProfesional.length > 0) {
        newErrors.servicios =
          "Todos los servicios deben tener un profesional asignado";
      } else {
    ...
    try {
      ...
      if (formData.fecha === hoyISO && formData.servicios.length > 0) {
        ...
        if (earliest <= ahoraMin) {
          const msg = `La hora debe ser posterior a ${ahora
            .toTimeString()
            .slice(0, 5)}`;
    ...
    try {
      ...
      console.log(
        "Iniciando búsqueda/creación de cliente para:",
        formData.cliente,
        formData.documento
      );
      let clientId;
      try {
        ...
        const errorMessage =
          ...
          error.message ||
          "Error al buscar o crear el cliente";
      ...
      if (serviciosInvalidos.length > 0) {
        toast.error(
          "Todos los servicios deben tener un profesional y servicio válido asignado"
        );
        setLoading(false);
        return;
      }
      ...
      const mapProfesionalToId = (servicio) => {
        ...
        const profesional = professionals.find(
          (p) => p.name === servicio.profesional
        );
        if (!profesional) {
          throw new Error(
            `No se encontró el profesional "${servicio.profesional}". Por favor, verifica que esté registrado.`
          );
        }
      ...
      const serviciosData = formData.servicios.map((s) => {
        ...
        if (!idEmpleado || isNaN(Number(idEmpleado))) {
          throw new Error(
            `El profesional del servicio "${s.nombre}" no tiene un ID válido`
          );
        }
      ...
      try {
        ...
        console.error(
          "Error response data (stringified):",
        ...
        if (error.response?.data) {
          ...
          if (statusCode === 409) {
            ...
            if (errorData.error) {
              ...
              baseMessage =
                "El empleado no tiene disponibilidad para la fecha y hora seleccionadas.";
            }
            ...
            let sugerencia =
              "Verifica que el empleado tenga programación asignada para esta fecha en la sección de Programación de Empleados.";
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary/50 ${
                    touchedFields.cliente && errors.cliente
                      ? "border-red-500"
                      : "border-gray-300"
                  }`}
              ...
              <div>
                ...
                <PhoneInput
                  ...
                  inputClass={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary/50 ${
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary/50 ${
                    touchedFields.fecha && errors.fecha
                      ? "border-red-500"
                      : "border-gray-300"
                  }`}
                />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div>
                ...
                <input
                  type="text"
                  value={convertirHoraA12Horas(resumen.inicio)}
                  readOnly
