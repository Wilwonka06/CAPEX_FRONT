    ...
    return (
      <div className="overflow-x-auto rounded-lg border border-gray-200 overflow-hidden shadow-sm bg-white font-inter">
        <div className="py-12 text-center">
          ...
          <p className="mt-4 text-gray-500 text-sm">No hay empleados registrados.</p>
            ...
            return (
              <tr key={employee.id} className="hover:bg-gray-50 transition-colors duration-150">
                ...
                <td className="py-4 px-4 text-xs text-gray-600 font-mono">
                  {employee.documento || employee.numero_documento || employee.num_documento || ''}
                </td>
                ...
                <td className="py-4 px-4 text-xs">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className={`text-xs font-semibold rounded-full px-2 py-1
                      ${employee.estado === 'Activo' ? 'bg-green-100 text-green-800' : ''}
                      ${employee.estado === 'Inactivo' ? 'bg-red-100 text-red-800' : ''}
                    `}>
                      {isToggling ? 'Cambiando...' : employee.estado}
                    </span>
                  </div>
                </td>
                <td className="py-4 px-4 text-xs font-medium text-right">
                  <div className="flex justify-end space-x-2">
                    ...
                    {/* Eliminación deshabilitada: usar inactivación mediante toggle */}
                  </div>
                </td>
              </tr>
            );
