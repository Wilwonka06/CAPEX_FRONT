...
import {
  ...
  codeFromLabel,
  toBackendDocCode,
} from "../../../../../shared/constants/documentTypes";

const tiposDocumento = DOC_TYPES_CODES.map((code) => ({
  value: code,
  label: `${code} - ${DOC_TYPE_LABELS[code]}`,
}));
  ...
  useEffect(() => {
    if (employee) {
      ...
      setForm({
        ...
        tipoDocumento: codeFromLabel(
          employee.tipo_documento || employee.tipoDocumento || "CC"
        ),
        documento:
          employee.documento ||
          employee.numero_documento ||
          employee.num_documento ||
          "",
    ...
    switch (name) {
      ...
      case "documento":
        if (!value.trim()) {
          ...
          const documentoErrors = validateEmployeeDocument(
            value,
            employees,
            employee
          );
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label
                  htmlFor="nombre"
                  className="block text-sm font-semibold text-gray-700 font-lato items-center gap-2"
                >
                  Nombre Completo *
                </label>
                <input
                  ...
                  className={`w-full border-2 rounded-xl px-2 py-2 text-gray-800 font-medium focus:outline-none focus:ring-2 focus:ring-[#FACC15] transition-all font-lato ${
                    errors.nombre
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={handleChange}
                  className="w-full border-2 rounded-xl px-2 py-2 text-gray-800 font-medium focus:outline-none focus:ring-2 focus:ring-[#FACC15] focus:border-[#FACC15] transition-all appearance-none bg-white font-lato"
                  disabled={isReadOnly}
                  required
                >
                  {DOC_TYPES_CODES.map((code) => (
                    <option
                      key={code}
                      value={code}
                    >{`${code} - ${DOC_TYPE_LABELS[code]}`}</option>
                  ))}
                </select>
              </div>
