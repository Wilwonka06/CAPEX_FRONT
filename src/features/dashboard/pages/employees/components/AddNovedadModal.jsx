...
const AddNovedadModal = ({ programaciones, onSave, onCancel, editing = null, showTitle = true }) => {
  ...
  useEffect(() => {
    if (editing) {
      ...
      const prog = programaciones.find(p => 
        String(p.id) === String(editing.id_programacion_recurrente) || 
        String(p.id_programacion_recurrente) === String(editing.id_programacion_recurrente)
      );
  ...
  const handleChange = (e) => {
    ...
    if (name === 'id_programacion_recurrente') {
      const prog = programaciones.find(p => 
        ...
        String(p.id_programacion_recurrente) === String(value)
      );
      setProgramacionSeleccionada(prog);
    }
  ...
  return (
    <div>
      ...
      <form id="novedad-form" onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          >
            ...
            {programaciones.map((prog) => {
              const progId = prog.id || prog.id_programacion_recurrente;
              const empleadoNombre = prog.usuario?.nombre || prog.Usuario?.nombre || 'Empleado no disponible';
              const horaEntrada = prog.hora_entrada || 'N/A';
              const horaSalida = prog.hora_salida || 'N/A';
              const diasSemana = prog.dias_semana || [];
              const diasMap = {0: 'Dom', 1: 'Lun', 2: 'Mar', 3: 'Mié', 4: 'Jue', 5: 'Vie', 6: 'Sáb'};
              const diasTexto = diasSemana.map(d => diasMap[d] || d).join(', ');
              
              return (
                <option key={progId} value={progId}>
                  {empleadoNombre} | {horaEntrada} - {horaSalida} | Días: {diasTexto}
                </option>
              );
            })}
          </select>
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
                <i className="bi bi-person-fill text-blue-600 text-lg"></i>
              </div>
              <div>
                <p className="text-sm font-semibold text-gray-800">Empleado:</p>
                <p className="text-base font-bold text-gray-900">
                  {programacionSeleccionada.usuario?.nombre || 
                   programacionSeleccionada.Usuario?.nombre || 
            ...
            <div className="mt-3 pt-3 border-t border-blue-200">
              <p className="text-xs text-gray-600 mb-1">
                <span className="font-medium">Horario:</span> {programacionSeleccionada.hora_entrada || 'N/A'} - {programacionSeleccionada.hora_salida || 'N/A'}
              </p>
              <p className="text-xs text-gray-600">
                <span className="font-medium">Días:</span> {
                  (programacionSeleccionada.dias_semana || []).map(d => {
                    const diasMap = {0: 'Dom', 1: 'Lun', 2: 'Mar', 3: 'Mié', 4: 'Jue', 5: 'Vie', 6: 'Sáb'};
                    return diasMap[d] || d;
                  }).join(', ')
                }
