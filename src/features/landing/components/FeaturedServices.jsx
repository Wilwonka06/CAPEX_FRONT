...
const FeaturedServices = () => {
  ...
  useEffect(() => {
    const loadServices = async () => {
      try {
        ...
        // Filtrar solo servicios activos y tomar los primeros 3
        const activeServices = data
          .filter((service) => service.active === true || service.estado === "Activo")
          .slice(0, 3);
        setServicios(activeServices);
      } catch (err) {
        console.error("[FeaturedServices] Error cargando servicios:", err);
        setServicios([]);
      } finally {
        setLoading(false);
      }
  ...
  const handleScheduleClick = (servicio) => {
    ...
    <section className="py-20 bg-gradient-to-br from-[#1E1E1E] to-[#2A2A2A] relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 relative z-10">
        ...
        {loading ? (
          ...
          <div className="flex justify-center items-center py-20">
            <div className="text-white/80 text-lg">No hay servicios disponibles</div>
          </div>
        ) : (
