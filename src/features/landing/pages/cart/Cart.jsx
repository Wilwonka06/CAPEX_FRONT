  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center py-10 px-2">
      ...
      <nav className="w-full max-w-6xl mx-auto text-xs text-gray-500 mb-6 flex items-center gap-2">
        ...
        <span className="mx-1">/</span>
        <span className="text-text-main font-semibold">Carrito</span>
      </nav>
      ...
      <div className="w-full max-w-6xl flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 bg-white rounded-2xl shadow-lg p-8">
                ...
                <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                  <td className="py-3 flex items-center gap-4">
                    <Link to={`/landing/productos/${item.id}`} className="group">
                        <img src={item.fotos && item.fotos.length > 0 ? item.fotos[0] : (item.foto || item.imagen)} alt={item.nombre} className="w-16 h-16 object-cover rounded bg-gray-100 group-hover:ring-2 group-hover:ring-primary transition" />
                    </Link>
                    <div>
          ...
          <div className="md:hidden flex flex-col gap-4">
            {cart.map(item => (
              <div key={item.id} className="flex gap-4 bg-gray-50 rounded-xl shadow p-4 items-center border border-gray-200">
                <Link to={`/landing/productos/${item.id}`} className="shrink-0">
                  <img src={item.fotos && item.fotos.length > 0 ? item.fotos[0] : (item.foto || item.imagen)} alt={item.nombre} className="w-20 h-20 object-cover rounded bg-white border border-gray-200" />
                </Link>
                <div className="flex-1 flex flex-col gap-1">
                  ...
                  <div className="text-primary font-bold text-lg mt-2">${formatNumber(item.precio * item.cantidad)}</div>
                </div>
        ...
        {/* Resumen de compra */}
        <div className="w-full md:w-96 bg-white rounded-2xl shadow-lg p-8 flex flex-col gap-6 border border-gray-100">
          <h2 className="text-xl font-bold text-primary mb-4 flex items-center gap-2"><i className="bi bi-receipt-cutoff"></i> Resumen de compra</h2>
          ...
          <div className="flex justify-between text-lg font-bold">
            <span>Total</span>
            <span className="text-primary">${formatNumber(subtotal)}</span>
          </div>
          <button
            onClick={() => navigate('/landing/checkout')}
            className="w-full py-3 bg-primary text-white rounded-lg font-bold text-lg shadow hover:bg-primary-dark transition mt-4 flex items-center justify-center gap-2"
          >
